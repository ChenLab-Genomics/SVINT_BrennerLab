...
# filter: use SVs that do not overlap protein-coding gene exons (for SVs overlapping gene introns but not the exons, annotate the overlapped genes as well as the RE-targets)
...
&SV_SUB::write_svint_anno($sv_file."_SVINT_v1.4_anno.vcf", $sv_file_new, $lines_a, $same_a, \%to_use, \%overlap_gene, \%overlap_exon, $sv2tad_bd, $sv2fi_tad);
&SV_SUB::write_txt_anno($sv_file."_SVINT_v1.4_anno.vcf", $fi_cut, $sv_file."_SVINT_v1.4_anno_FI$fi_cut\_impact100_2methods.txt", $sv_file."_SVINT_v1.4_anno_FI$fi_cut\_impact100_1method.txt",$sv_file."_SVINT_v1.4_anno_FI$fi_cut\_impact50_2methods.txt", $sv_file."_SVINT_v1.4_anno_FI$fi_cut\_impact50_1method.txt");
print "SVINT done.\n################\n";
